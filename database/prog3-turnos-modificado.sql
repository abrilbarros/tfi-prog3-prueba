...
--
-- Servidor: 127.0.0.1
-- Tiempo de generación: 16-06-2026 a las 08:37:51
...
-- Procedimientos
--
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_estadisticas_atenciones` (IN `p_fecha_desde` DATE, IN `p_fecha_hasta` DATE)   BEGIN
    SELECT 
        COUNT(*) AS total_turnos,
        SUM(CASE WHEN atendido = 1 THEN 1 ELSE 0 END) AS total_atendidos,
        SUM(CASE WHEN atendido = 0 AND activo = 1 THEN 1 ELSE 0 END) AS total_pendientes,
        SUM(valor_total) AS monto_total,
        AVG(valor_total) AS promedio_consulta
    FROM turnos_reservas
    WHERE DATE(fecha_hora) BETWEEN p_fecha_desde AND p_fecha_hasta
        AND activo = 1;
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_estadisticas_por_medico` (IN `p_fecha_desde` DATE, IN `p_fecha_hasta` DATE)   BEGIN
    SELECT 
        m.id_medico,
        u.apellido,
        u.nombres,
        COUNT(t.id_turno_reserva) AS total_atenciones,
        SUM(CASE WHEN t.atendido = 1 THEN 1 ELSE 0 END) AS atendidos,
        SUM(t.valor_total) AS monto_generado
    FROM medicos m
    INNER JOIN usuarios u ON m.id_usuario = u.id_usuario
    LEFT JOIN turnos_reservas t ON m.id_medico = t.id_medico
        AND DATE(t.fecha_hora) BETWEEN p_fecha_desde AND p_fecha_hasta
        AND t.activo = 1
    GROUP BY m.id_medico, u.apellido, u.nombres
    ORDER BY total_atenciones DESC;
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_estadisticas_por_obra_social` (IN `p_fecha_desde` DATE, IN `p_fecha_hasta` DATE)   BEGIN
    SELECT 
        os.id_obra_social,
        os.nombre,
        COUNT(t.id_turno_reserva) AS total_turnos,
        SUM(t.valor_total) AS monto_total,
        AVG(t.valor_total) AS promedio
    FROM obras_sociales os
    LEFT JOIN turnos_reservas t ON os.id_obra_social = t.id_obra_social
        AND DATE(t.fecha_hora) BETWEEN p_fecha_desde AND p_fecha_hasta
        AND t.activo = 1
...
(1, 'PARTICULAR', 'Persona sin obra social', 10.00, 1, 1),
(2, 'OSUNER', 'osu', 10.00, 0, 1),
(3, 'OSECAC', 'ose', 11.00, 0, 1),
(4, 'OSUNER 3', 'OSU', 13.00, 0, 0),
(7, 'PAMI', 'jubilados/pensionados', 40.00, 0, 1),
(10, 'OSPE', 'Cobertura para trabajadores de estaciones de servicio', 20.00, 0, 0),
(11, 'UOCRA Salud', 'Cobertura para trabajadores de la construcción', 15.00, 0, 1);
...
(8, 3, 2, 2, '2026-12-25 15:30:00', 9000.00, 0, 0);
...
--
ALTER TABLE `obras_sociales`
  MODIFY `id_obra_social` int(10) UNSIGNED NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=12;
...
--
ALTER TABLE `turnos_reservas`
  MODIFY `id_turno_reserva` int(10) UNSIGNED NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=9;
